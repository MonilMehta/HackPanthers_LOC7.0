...
const StatCard = ({ title, value, description, icon, trend, trendValue, link }) => (
  <Card className="relative overflow-hidden">
    ...
    <CardContent>
      ...
      {link && (
        <Link to={link} className="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity bg-black/5 flex items-center justify-center">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card className="col-span-2">
          ...
          <CardContent className="space-y-4">
            {[
              { time: "2 hours ago", type: "Theft", location: "Sector 18", status: "Active" },
              { time: "4 hours ago", type: "Assault", location: "Sector 24", status: "Under Investigation" },
              { time: "6 hours ago", type: "Cybercrime", location: "Sector 62", status: "Evidence Collection" },
              { time: "12 hours ago", type: "Burglary", location: "Sector 15", status: "Solved" }
            ].map((case_, i) => (
              <div key={i} className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-4">
                  <div className="text-muted-foreground min-w-[100px]">{case_.time}</div>
                  <div>{case_.type}</div>
                  <div className="text-muted-foreground">{case_.location}</div>
                </div>
                <div className={`px-2 py-1 rounded-full text-xs ${
                  case_.status === 'Solved' ? 'bg-green-100 text-green-800' :
                  case_.status === 'Active' ? 'bg-blue-100 text-blue-800' :
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                ...
                { type: "WARNING", text: "Suspicious activity reported in Sector 12" },
                { type: "NOTICE", text: "Community meeting scheduled for tomorrow" }
              ].map((bulletin, i) => (
                <div key={i} className="flex items-start gap-2 text-sm">
                  <span className={`px-2 py-1 rounded text-xs ${
                    bulletin.type === 'ALERT' ? 'bg-red-100 text-red-800' :
                    bulletin.type === 'WARNING' ? 'bg-yellow-100 text-yellow-800' :
